...
- **半连接队列（SYN queue）**：收 SYN、回了 SYN+ACK、还在等最终 ACK 的连接（`SYN_RECV`），
  上限是 `net.ipv4.tcp_max_syn_backlog`（每监听器）。
- **全连接队列（accept queue）**：已完成握手、等应用 `accept()` 的连接，上限是
  `min(listen backlog, net.core.somaxconn)`。
- **`somaxconn`**：内核级天花板。Linux 5.4 起默认 **4096**，更早是 **128**——这个默认值
  是很多"连接莫名丢弃"事故的根因。
- **SYN Cookie**：半连接队列溢出时的回退机制，把连接状态编码进 SYN+ACK 的序号里，不占队列。
- **`tcp_abort_on_overflow`**：全连接队列满时"丢弃等重传"还是"直接回 RST"的开关。

历史背景：Linux **2.2** 把 `backlog` 的语义从"未完成连接数"改为"已完成连接数"（man7
`listen(2)`）。
...
的连接失败，而连接失败又引发客户端重传 SYN，反过来给半连接队列加压。
...
| 半连接 | `net.ipv4.tcp_max_syn_backlog` | 每监听器限制；低内存机器最小 128 |

man7 `listen(2)`：「If the backlog argument is greater than the value in
`/proc/sys/net/core/somaxconn`, then it is silently capped to that value.」
所以**只改应用层 `listen(fd, 1024)` 没用**，必须同时调 `somaxconn`。

### 3. 溢出时的行为

**半连接队列溢出**：

- `tcp_syncookies = 1`（默认）→ 不再入队，改用 SYN Cookie；内核文档特别注明：syncookies 下
  `tcp_max_syn_backlog` 的上限检查**不再严格适用**。
- `tcp_syncookies = 0` → 直接丢 SYN，客户端只能重传（`tcp_syn_retries`）。
- 典型信号：`dmesg` 出现 `Possible SYN flooding on port ...`。**这不一定是攻击**，
  合法突发流量也会触发。
...
| 取值 | 动作 | 客户端观感 |
| --- | --- | --- |
| `0`（默认） | 静默忽略客户端的最终 ACK；服务端按 `tcp_synack_retries` 重传 SYN+ACK，客户端重发 ACK | 稍慢，只要应用及时 `accept()` 就能**自愈** |
...
内存代价：内核文档给出**一个 `SYN_RECV` 请求套接字约 304 字节**，所以
`tcp_max_syn_backlog = 65536` 约占用 **19 MiB**；`= 128`（低内存最小值）只要 38 KiB。
...
### C（`backlog_impl.h` 由 `main.c` 内部 `#include`，编译只列 `main.c`）
...
### Go（同包多文件，用 `.` 让 go 收全包：main.go + backlog_model.go）
...
```c
/* 生效上限 = backlog 与 somaxconn 取小（静默截断） */
limit = l->backlog < l->somaxconn ? l->backlog : l->somaxconn;

/* 半连接队列溢出 → syncookies → 丢弃（三级回退） */
if (l->syn_q < l->tcp_max_syn_backlog) return ACT_SYNACK_SENT;
if (l->syncookies)                     return ACT_SYNCOOKIE_SENT;
return ACT_SYN_DROPPED;

/* 全连接队列溢出：0=静默忽略最终 ACK（等重传自愈）1=直接 RST（不再自愈） */
```

```python
# 真实 TCP 重传是**指数退避**的（约 1s/2s/4s/8s/16s/32s）。用固定间隔重试会让
# "自愈"几乎不可能发生，从而把"应用太慢"误判成"协议不自愈"（这是本轮踩到过的坑）。
retry_delay = lambda base, attempt: base * (2 ** attempt)
```
...
1. **`backlog` 不是"能同时服务的连接数"**：它只是"握完手还没被 accept 的缓冲区"。
   队列满说明**应用消费速度跟不上握手速率**，不是带宽或算力问题。
2. **`somaxconn` 才是天花板**：`listen(fd, 1000000)` 不会报错，只会被静默截到
   `somaxconn`。上线前务必确认这个值（老发行版默认 128 太容易撞）。
3. **全连接队列溢出的表现极具误导性**：客户端 `SYN` 重传但握手其实已完成，服务端业务日志
   里看不到任何请求 → 容易误去查带宽/负载。**先看 `ss -lnt` 的 `Recv-Q`**。
4. **别把 `tcp_abort_on_overflow=1` 当"优化"**：它只是把"慢"换成"快失败"，反而减少成功
   连接数（本 demo 实测 27 → 12）。
5. **别用 syncookies 扛合法流量**：内核文档明确反对。它禁用 TCP 扩展（窗口缩放、SACK、
   时间戳），高 RTT/大带宽场景会严重降级。正确做法是加大 `tcp_max_syn_backlog` 并让应用
   `accept()` 更快。
6. **建模时别用固定间隔重试**：真实重传是指数退避的。第一版模拟用固定 3 个 tick 重试，
   结果"自愈"完全没发生（5 次重试只覆盖 15 个 tick，而队列排空需要几百个 tick），
   差点得出"默认策略不自愈"的错误结论。顺带一提：曾用于加速 TIME_WAIT 回收的
   `tcp_tw_recycle` 在 NAT 下会丢连接，Linux 4.12 起已移除；现在用 `tcp_tw_reuse`。

## 参考资料（实际阅读过的权威来源）

- [listen(2) — Linux manual page](https://man7.org/linux/man-pages/man2/listen.2.html)
  — `backlog` 在 Linux 2.2 后的语义、被 `somaxconn` 静默截断、5.4 起默认 4096（此前 128）。
- [Linux 内核文档 ip-sysctl](https://docs.kernel.org/networking/ip-sysctl.html)
  — `tcp_abort_on_overflow`（默认 FALSE）、`tcp_max_syn_backlog`（per-listener、每个
  SYN_RECV 约 304 字节）、`tcp_syncookies`（"seriously violate TCP protocol"、"MUST NOT
  be used to help highly loaded servers"）、`somaxconn`、`tcp_tw_reuse`。
