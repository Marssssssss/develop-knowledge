...
2. 运行期：node 直跑 mapped_types.ts，要求全部运行时断言通过。
...
MAIN = os.path.join(HERE, "mapped_types.ts")
...
def check_runtime():
    ...
    print("[ok] mapped_types.ts 运行时断言 %d 条全绿" % len(lines))


def main():
    check_type_fixtures()
    check_runtime()
    print("=== 映射类型与修饰符 demo 自检全部通过 ===")
    return 0
