...
历史坑(2026-09-18 第二次修复):
  C/Go 分支缺**单引号字符/符文字面量**处理,于是 Go 里的符文字面量 `` '`' ``
  (例如 `switch c { case '"', '\'', '`': }`)会被当成「原始字符串的起始反引号」,
  一路吞到文件中下一个反引号为止 —— 若两者相距上千字节,中间的真实代码(含括号)全被置空,
  表现为莫名其妙的 `MISMATCH ')'`。修法是给非 Python 分支补 `'` 与 `"` 同样规格的
  跳转分支:走进任一引号字面量后直接跳到其收尾引号,于是反引号分支只会在真正处于
  **代码位置**时才被触发。
