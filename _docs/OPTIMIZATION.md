...
由调度环境创建的巡检任务,prompt 中显式约束(2026-09-12 与当时「5 主/轮」配额同步放宽;2026-09-25 配额降 1 主/轮后,搜索上限沿用历史值作天花板、不收紧):
- 不展开调研综述类问题
- 单轮最多调用 5 次 `agentic_search`(天花板,非目标)
- 单轮最多调用 15 次 `WebSearch` + 10 次 `WebFetch`(天花板,非目标);新 demo 每轮 ≤ 1
...
| 日期 | 变更 |
| --- | --- |
| 2026-09-25 | §2.3 注释更新:配额 5 主 → 1 主/轮(用户指示「每次只推进一个条目」),新 demo 上限 ≤5 → ≤1;搜索上限(≤5/≤15/≤10)沿用历史值作天花板、不收紧 |
| 2026-09-11 | 首版定义 |
