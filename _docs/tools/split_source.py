...
拆完之后必做的三步（搬运只动行，**符号可见性不会自动修好**）：
  1. **从被拆文件里删掉已搬走符号专用的 import** —— Go 的未用 import 是硬编译
     错误（如 `HistIndex` 搬到新文件后，主文件里的 `math/bits` 必须删）。
  2. **给新文件补 import 块**：把搬运段实际用到的名字从原文件抄过去（Python 用
     `from <mod> import (...)`，Go 同包免 import，C 用 `#include`）。
     可先用脚本统计"搬运段引用了哪些顶层符号"再对齐，别凭印象。
  3. **复核同目录顶层符号无重复**（Go 同包重名是编译错误）、**C 实现头无重复包含**、
     并跑一次自检（Python 直接执行；C/Go 无工具链则人工读一遍函数签名与调用实参个数）。
     `syntax_sanity.py` 只查括号配平/未用 import/CRLF，**不查行数**，行数要单独核。
"""
