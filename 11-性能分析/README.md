...
- [x] 05-容量规划与性能建模（排队论 / 扩展性定律 / 余量论证，2026-09-15 S2 建目录 → 2026-09-24 首批 5 demo，ID 655-659）
- [x] 06-数据库性能（PostgreSQL EXPLAIN cost 口径 / 索引选择率 / 连接池 / 慢查询归因，2026-09-21 S2 建目录 → 2026-09-25 06:00 槽首批 5 demo，ID 718-722：选择率五公式 / Index-Only Scan 与 VM / 等待事件归因 / 连接池非单调 / 泛化计划翻转）
...
- [x] USL 参数拟合（从实测 (N, X) 点估 α、β、γ 并定位 Nmax）（[05-容量规划与性能建模/USL三参数拟合与峰值定位/](./05-容量规划与性能建模/USL三参数拟合与峰值定位/)，2026-09-24，ID 657）
- [x] 容量规划与性能建模首批：Little 定律与事件驱动仿真实证、M/M/c 与 Erlang C 拐点、USL 三参数拟合与峰值定位、Amdahl/Gustafson/USL 三条扩展律的关系、过载保护与自适应并发（[05-容量规划与性能建模/](./05-容量规划与性能建模/)，2026-09-24 首批 5 demo，ID 655-659）
- [ ] M/M/c 的尾延迟闭式与多队列调度选择（均值口径已在 ID 656 覆盖，尾延迟未做）
- [ ] 排队网络与容量分配（Jackson 网络 / Kingman 近似）
