...
2. 运行期：node 直跑 conditional_types.ts，要求全部运行时断言通过。
...
MAIN = os.path.join(HERE, "conditional_types.ts")
...
def check_runtime():
    ...
    print("[ok] conditional_types.ts 运行时断言 %d 条全绿" % len(lines))


def main():
    check_type_fixtures()
    check_runtime()
    print("=== 条件类型与分发 demo 自检全部通过 ===")
    return 0
