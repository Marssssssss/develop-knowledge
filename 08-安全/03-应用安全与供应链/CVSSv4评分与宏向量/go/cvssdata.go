// CVSS v4.0 常量表：指标层级、宏向量得分、最高严重度向量与深度。
//
// 数据来自官方参考实现 FIRSTdotorg/cvss-v4-calculator 的
// cvss_score.js（层级）、cvss_lookup.js（270 个宏向量得分）、
// max_composed.js（最高严重度向量）、max_severity.js（深度）、
// metrics.js（指标顺序与合法取值）。本文件只放数据，算法见 cvss.go。
package main

// 各指标取值 -> 严重度层级（越小越严重）；SC 没有 S 取值，SI/SA 有
var levels = map[string]map[string]float64{
...
// 合法取值（含 X = Not Defined），切片顺序即向量串中的固定顺序
var valid = []struct {
	name   string
	values string
}{
	...
	{"MAV", "XNALP"}, {"MAC", "XLH"}, {"MAT", "XNP"},
	{"MPR", "XNLH"}, {"MUI", "XNPA"},
	{"MVC", "XHLN"}, {"MVI", "XHLN"}, {"MVA", "XHLN"},
	{"MSC", "XHLN"}, {"MSI", "XSHLN"}, {"MSA", "XSHLN"},
...
// 各 EQ 的最高严重度向量（max_composed.js）
var eq1Max = map[int][]string{
	0: {"AV:N/PR:N/UI:N/"},
	1: {"AV:A/PR:N/UI:N/", "AV:N/PR:L/UI:N/", "AV:N/PR:N/UI:P/"},
	2: {"AV:P/PR:N/UI:N/", "AV:A/PR:L/UI:P/"},
}

var eq2Max = map[int][]string{
	0: {"AC:L/AT:N/"},
	1: {"AC:H/AT:N/", "AC:L/AT:P/"},
}

var eq3eq6Max = map[int]map[int][]string{
	0: {
...
var eq4Max = map[int][]string{
	0: {"SC:H/SI:S/SA:S/"},
	1: {"SC:H/SI:H/SA:H/"},
	2: {"SC:L/SI:L/SA:L/"},
}

var eq5Max = map[int][]string{0: {"E:A/"}, 1: {"E:P/"}, 2: {"E:U/"}}

// 宏向量深度 +1（max_severity.js），乘 step 后作为插值分母
type depthTable struct {
	eq1    map[int]float64
	eq2    map[int]float64
	eq3eq6 map[int]map[int]float64
	eq4    map[int]float64
	eq5    map[int]float64
}

var depth = depthTable{
	eq1: map[int]float64{0: 1, 1: 4, 2: 5},
	eq2: map[int]float64{0: 1, 1: 2},
	eq3eq6: map[int]map[int]float64{
		0: {0: 7, 1: 6},
		1: {0: 8, 1: 8},
		2: {1: 10},
	},
	eq4: map[int]float64{0: 6, 1: 5, 2: 4},
	eq5: map[int]float64{0: 1, 1: 1, 2: 1},
}
