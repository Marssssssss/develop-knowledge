...
> 规则:每个定点槽(全天 3 槽:`03:00 11:00 19:00`,单条 cron 任务 `0 */8 * * *`)巡检完成后,把本轮"添加了什么知识"通过企业微信群机器人 webhook 推送给用户。
...
┌──────────────────────────────────────────────────────────────┐
│ 定时巡检(automation)                                        │
│   1. 定位领域 → 2. 查权威 → 3. 写 1 个 demo → 4. 更新索引     │
...
`+N` — 本轮新增 demo 数(2026-09-25 起 0/1;配额为 5 的历史轮次曾 0-5)。
...
| 日期 | 变更 |
|---|---|
| 2026-09-25 | 配额 5→1 demo/轮(用户指示「每次只推进一个条目」):首段调度描述改为单条 cron 3 槽、第一节流程图「写 5 个 demo」→「写 1 个 demo」、第六节「+N」范围改 0/1 |
